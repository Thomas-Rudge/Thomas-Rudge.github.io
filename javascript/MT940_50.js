...
document.addEventListener('DOMContentLoaded', function() {
   ...
   // This will get the values from the user input area and
   // the advanced options, and then pass them to the main function
   var swiftButton = document.getElementById('swift_button');
   ...
   // This displays and hides the advanced message options
   var optionButton = document.getElementById('options_button');
   optionButton.addEventListener('click', function() {
      var sop = document.getElementById("swift_options");

      if(sop.style.display == "block") {
         sop.style.display = "none";
      } else {
         sop.style.display = "block";
      }

      toggleButtonName("options_button", '☰ more options', '☷ less options');

      return false;
   });

   // This function changes the text of a link by ID
   var toggleButtonName = function(linkID, startText, altText) {
      var butname = document.getElementById(linkID);

      if(butname.text === startText) {
         butname.text = altText;
      } else {
         butname.text = startText;
      }
   };

   // This function ensures only options relevant to the message
   ...
   var directionOptions = function() {
      ...
      if (direction === "I") {
         ...
         document.getElementById("out0").disabled = true;
         ...
         document.getElementById("in0").disabled = true;
         document.getElementById("in1").disabled = true;
         document.getElementById("out0").disabled = false;
   ...
   var slctDrctn = document.getElementById('swift_direction');
   slctDrctn.addEventListener('change', directionOptions);
   ...
   var genMt9 = function(activeFile, config) {
      ...
      if (activeFile[0].toUpperCase().replace(/ /g, '').startsWith('SENDERBIC,RECEIVERBIC,ACCOUNTID,\
                                                                    STMTNO,STMTPG,OPBALSIGN')) {
         activeFile.shift();
      }
